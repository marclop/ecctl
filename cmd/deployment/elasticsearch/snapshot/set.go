...
const setsnapshotLong = `Manages the snapshot settings of an Elasticsearch cluster.
Note that each operation is add/modify only, unspecified existing snapshot settings will be unchanged.`

var setsnapshotExamples = `
...
var setCmd = &cobra.Command{
	...
	Long:    setsnapshotLong,
	Example: setsnapshotExamples,
	PreRunE: cmdutil.MinimumNArgsAndUUID(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		filename, _ := cmd.Flags().GetString("file")
		var req models.ClusterSnapshotSettings
		if err := cmdutil.DecodeFile(filename, &req); err != nil {
			return err
		}

		res, err := snapshot.SetSettings(snapshot.SetSettingsParams{
			API:       ecctl.Get().API,
			ClusterID: args[0],
			Request:   &req,
		})
...
func init() {
	...
	setCmd.Flags().StringP("file", "f", "", "JSON file that contains JSON-style domain-specific snapshot settings")
